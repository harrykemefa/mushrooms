		...
		if ( isset( $get['error'] ) ) { // error comming from facebook GET vars
			...
			if ( $accessTokenInfo['has_errors'] ) { // there was an error getting an access token with the code
				...
				if ( !$fbUserInfo['has_errors'] && !empty( $fbUserInfo['fb_response']['id'] ) && !empty( $fbUserInfo['fb_response']['email'] ) ) { // facebook gave us the users id/email
					...
					$fb_responseid = $fbUserInfo['fb_response']['id'];
					$query1 = $db->query("SELECT * FROM users WHERE fb_user_id = '$fb_responseid'");
					$userInfoWithId = mysqli_fetch_assoc($query1);

					// check for user with email
					$fb_responsemail = $fbUserInfo['fb_response']['email'];
					$query2 = $db->query("SELECT * FROM users WHERE email = '$fb_responsemail'");
					$userInfoWithEmail = mysqli_fetch_assoc($query2);

					if ( $userInfoWithId || ( $userInfoWithEmail && !$userInfoWithEmail['password'] ) ) { // user has logged in with facebook before so we found them
						// update user
						global $db;
						$userinfo_id = $userInfoWithId['id'];
						$fb_accesstoken = $_SESSION['fb_access_token'];
						$db->query("UPDATE users SET fb_access_token = '$fb_accesstoken' WHERE id = '$userinfo_id'");
						$userinfo_query = $db->query("SELECT * FROM users WHERE id = '$userinfo_id'");
						$userInfo = mysqli_fetch_assoc($userinfo_query);

						// save info to php session so they are logged in
						$_SESSION['is_logged_in'] = true;
						$_SESSION['user_info'] = $userInfo;
						$_SESSION['fb_log_true'] = true;
						...
						$first_name = $fbUserInfo['fb_response']['first_name'];
						$last_name = $fbUserInfo['fb_response']['last_name'];
						$fb_user_id = $fbUserInfo['fb_response']['id'];
						$fb_accesstoken = $_SESSION['fb_access_token'];
						// $userId = signUserUp( $fbUserInfo['fb_response'] );
					   $signUserUp = $db->query("INSERT INTO users (email,first_name,last_name,password,fb_user_id,fb_access_token) VALUES ('$email','$first_name','$last_name','','$fb_user_id','$fb_accesstoken')");
					    $userId = mysqli_insert_id($db);
					    $query3 = $db->query("SELECT * FROM users WHERE id = '$userId'");
					    $userInfo = mysqli_fetch_assoc($query3);

						// save info to php session so they are logged in
						$_SESSION['is_logged_in'] = true;
						$_SESSION['user_info'] = $userInfo;
						$_SESSION['fb_sign_true'] = true;
